...
def draw_phase_bar(screen, W, phase_name, t_left, t_total, color):
    ...
    label = pygame.font.SysFont(None, 24, bold=True).render(
        f"{phase_name} — {t_left:4.1f}s", True, (18,20,26)
    )
    bar.blit(label, (10,4))
...
def draw_goal_label(screen, W, H, phase, cfg, angle):
    ...
    if phase == "FRENTE":
        ...
        msg = f"Meta TRÁS: ≥ {cfg['target_back']:.0f}°"
        hit = (abs(angle) >= cfg["target_back"])
...
def draw_session_info(screen, W, reps_done, reps_total, elapsed_s, vavg_kmh):
    ...
    label = pygame.font.SysFont(None, 24).render(
        f"Reps {reps_done}/{reps_total}  •  Tempo {mm:02d}:{ss:02d}  •  Vmé dia {vavg_kmh:4.1f} km/h",
        True, (255,255,255)
    )
    bar.blit(label, (12, 2))
    screen.blit(bar, (0, 0))
...
def draw_report(screen, W, H, report, total_time, vavg_kmh, restart_btn_rect):
    ...
    y = 140
    header = pygame.font.SysFont(None, 28, bold=True).render(
        "Rep  Direção   Ângulo extremo (°)   Tempo p/ Meta (s)", True, (255,255,255)
    )
        ...
        for e in pygame.event.get():
            if e.type == pygame.QUIT:
                ...
                if e.key == pygame.K_ESCAPE:
                    ...
                    if started and not session_done:
                        paused = not paused
